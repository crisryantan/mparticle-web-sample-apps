...
*/
      initForwarder: function (
          ...
          // Don’t load the script in test mode
          if (testMode) {
              console.log('Test mode enabled – skipping Rokt launcher script load.');
              return;
          }

          // If the Rokt script has already been loaded or the promise is set, do nothing
          if (!window.RoktLauncherScriptPromise) {
              window.RoktLauncherScriptPromise = new Promise((resolve, reject) => {
                  var target = document.head || document.body;
                  var script = document.createElement('script');
                  script.type = 'text/javascript';
                  // Replace this with your actual script URL if different from forwarderSettings
                  ...
                  script.addEventListener('load', function () {
                      ...
                      if (window.Rokt && typeof window.Rokt.createLauncher === 'function') {
                          // Create the launcher and store it in a Promise
                          window.Rokt.currentLauncher = window.Rokt.createLauncher({
                              accountId: forwarderSettings.accountId || '2836261467443182542',
                              sandbox: 'true',
                          })
                              .then(function (launcher) {
                                  // Process any queued events now that Rokt is ready
                                  ...
                                  // Resolve the script-level promise with the launcher
                                  resolve(launcher);
                                  // Return the launcher so window.Rokt.currentLauncher also resolves
                                  return launcher;
                              })
                              .catch(function (err) {
                                  console.error('Error creating Rokt launcher:', err);
                                  reject(err);
                                  throw err; // ensure the Promise rejects correctly
                              });
                      } else {
                          var msg = 'Rokt object is not available after script load.';
                          console.error(msg);
                  ...
                  // Finally, append the script to load it
                  target.appendChild(script);
              });
          } else {
              console.log('Rokt launcher script is already loading or has loaded.');
          }
      },
};
